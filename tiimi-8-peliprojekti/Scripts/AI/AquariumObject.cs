...
public partial class AquariumObject : Node2D
{
	...
	{
		if(GameManager.Instance.ActiveAquarium.MaxIdealOxygen()) // If the aquarium's oxygen is outside of ideal range fish takes damage over time
		{
	...
	{
		//Health
		if(_health < _maxHealth && GameManager.Instance.ActiveAquarium.MaxIdealOxygen()) // If the aquarium's oxygen is in ideal range fish heals over time
		{
	...
	{
		GameManager.Instance.ActiveAquarium.RemoveObject(this); // Removes fish from aquarium
		if(objectCells != null)
		{
